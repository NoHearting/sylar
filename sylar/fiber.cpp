...
Fiber::~Fiber(){
    ...
    --s_fiber_count;
    if(m_stack){
        SYLAR_ASSERT(m_state==TERM || m_state == INIT || m_state == EXECPT);
...
void Fiber::MainFunc(){
    ...
    try{
        ...
        cur->m_state = EXECPT;
        SYLAR_LOG_ERROR(g_logger) << "Fiber Execption :"<< e.what()
                <<" fiber_id = " << cur->getId()
                <<std::endl
    ...
    SYLAR_ASSERT2(false,"never reback! fiber_id="+std::to_string(raw_ptr->getId()));
}
