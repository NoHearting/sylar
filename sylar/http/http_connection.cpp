 ...
 * @LastEditors: zsj
 * @LastEditTime: 2020-06-21 16:13:58
 */ 
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req
    ...
    if(!rsp){
        std::make_shared<HttpResult>((int)HttpResult::Error::TIMEOUT
...
HttpConnection::ptr HttpConnectionPool::getConnection(){
    ...
    if(ptr != nullptr){
        ...
        if(sock->connect(addr) == false){
            SYLAR_LOG_ERROR(g_logger) << "sock connect fail:" << m_host;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr,HttpConnectionPool * pool){
    ++ptr->m_request;
    // 
    if((ptr->isConnected() == false) || (ptr->m_request >= pool->m_maxRequest)
...
HttpResult::ptr HttpConnectionPool::doRequest(HttpRequest::ptr req,uint64_t timeout_ms){
    ...
    if(!conn){
        std::make_shared<HttpResult>((int)HttpResult::Error::POOL_GET_CONNECTION
                    ,nullptr,"pool host: "+m_host + " port:"+std::to_string(m_port));
    }
    auto sock = conn->getSocket();
    if(!sock){
        std::make_shared<HttpResult>((int)HttpResult::Error::POOL_INVALID_CONNECTION
                    ,nullptr,"pool host: "+m_host + " port:"+std::to_string(m_port));
    }
    sock->setRecvTimeout(timeout_ms);
    // HttpConnection::ptr conn = std::make_shared<HttpConnection>(sock);
    ...
    if(!rsp){
        std::make_shared<HttpResult>((int)HttpResult::Error::TIMEOUT
                    ,nullptr,"recv response timeout:"+sock->getRemoteAddress()->toString()+" timeout_ms" 
                    + std::to_string(timeout_ms));
    }
    return std::make_shared<HttpResult>((int)HttpResult::Error::OK,rsp,"OK");
}
