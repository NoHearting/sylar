 ...
 * @version: 
 * @Author: zsj
 * @Date: 2020-06-23 16:52:48
 * @LastEditors: zsj
 * @LastEditTime: 2020-06-29 11:23:46
 */ 
...
std::pair<HttpResult::ptr,WSConnection::ptr> WSConnection::Create(Uri::ptr uri
    ...
    if(!sock){
        return std::make_pair(std::make_shared<HttpResult>(
            (int)HttpResult::Error::CREATE_SOCKET_ERROR
            ,nullptr
    ...
    if(!sock->connect(addr)){
        return std::make_pair(std::make_shared<HttpResult>(
            (int)HttpResult::Error::CONNECT_FAIL
            ,nullptr,
