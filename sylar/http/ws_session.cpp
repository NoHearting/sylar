 ...
 * @version: 
 * @Author: zsj
 * @Date: 2020-06-28 19:56:20
 * @LastEditors: zsj
 * @LastEditTime: 2020-06-29 20:06:59
 */ 
...
HttpRequest::ptr WSSession::handleShake(){
    ...
    do{
        ...
        if(req->getHeaderAs<int>("Sec-webSocket-Version") != 13){
            SYLAR_LOG_INFO(g_logger) << "http header Sec-webSocket-Version";
            break;
        }

        std::string key = req->getHeader("Sec-WebSocket-Key");
        if(key.empty()){
            SYLAR_LOG_INFO(g_logger) << "http header Sec-WebSocekt-Key = null";
            break;
        }
        ...
        rsp->setReason("Web Socket Protocol Hanshake");
...
WSFrameMessage::ptr WSRecvMessage(Stream * stream,bool client){
    ...
    do{
        ...
        if(ws_head.opcode == WSFrameHead::PING){
        ...
        else if(ws_head.opcode == WSFrameHead::CONUINUE
            ...
            if(!opcode && ws_head.opcode != WSFrameHead::CONUINUE){
