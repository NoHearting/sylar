 ...
 * @version:
 * @Author: zsj
 * @Date: 2020-11-02 01:22:28
 * @LastEditors: zsj
 * @LastEditTime: 2020-11-02 02:11:30
 *
 */
...
static sylar::ConfigVar<uint32_t>::ptr g_rock_protocol_gzip_min_length =
    sylar::Config::Lookup("rock.protocol.gzip_min_length", (uint32_t)(1024 * 4),
...
bool RockNotify::parseFromByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        SYLAR_LOG_ERROR(g_logger) << "RockNotify serilaizeToByteArray error";
    }
    return false;
}


static const uint32_t s_rock_magic[2] = {0xab, 0xcd};
...
int32_t RockMessageDecoder::serializeTo(Stream::ptr stream, Message::ptr msg) {
    ...
    if ((uint32_t)header.length >=
        ...
        if (zstream->write(ba, -1) != Z_OK) {
            SYLAR_LOG_ERROR(g_logger)
                << "RockMessageDecoder serilaize to gzip error";
            return -1;
        }
        if (zstream->flush() != Z_OK) {
            SYLAR_LOG_ERROR(g_logger)
                << "RockMessageDecoder serialize to gzip error";
            return -2;
        }
    ...
    if (stream->writeFixSize(&header, sizeof(header)) <= 0) {
        SYLAR_LOG_ERROR(g_logger)
            << "RockMessageDecoder serilize to write header fail";
        return -3;
    }

    if (stream->writeFixSize(ba, -1) <= 0) {
        SYLAR_LOG_ERROR(g_logger)
            << "RockMessageDecoder serilize to write body fail";
        return -4;
    }
