...
bool Socket::connect(const Address::ptr addr,uint64_t timeout_ms){
    ...
    if(timeout_ms == (uint64_t)-1){
        ...
        if(::connect_with_timeout(m_sock,addr->getAddress(),addr->getAddressLen(),timeout_ms)){
            SYLAR_LOG_ERROR(g_logger) << "sock="<<m_sock<<" connect("<<addr->toString()
                <<")timeout="<<timeout_ms
                <<") error errno="<<errno <<" errstr="<<strerror(errno);
            close();
...
int Socket::recvFrom(void * buffer,size_t length,Address::ptr from,int flags){
    if(isConnected()){
        ...
        return ::recvfrom(m_sock,buffer,length,flags,from->getAddress(),&len);
    }
    return -1;
}
...
Address::ptr Socket::getRemoteAddress(){
    ...
    if(getpeername(m_sock,result->getAddress(),&addrlen)){
        SYLAR_LOG_ERROR(g_logger) << "getpeeername error sock=" << m_sock  
