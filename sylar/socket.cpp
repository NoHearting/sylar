 ...
 * @LastEditors: zsj
 * @LastEditTime: 2020-10-13 16:06:14
 */
#include "socket.h"
...
bool Socket::bind(const Address::ptr addr) {
    ...
    if (::bind(m_sock, addr->getAddress(), addr->getAddressLen())) {
        SYLAR_LOG_ERROR(g_logger)
            << "bind errorno=" << errno << " strerr=" << strerror(errno);
        return false;
    }
    getLocalAddress();
    return true;
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if (timeout_ms == (uint64_t)-1) {
        if (::connect(m_sock, addr->getAddress(), addr->getAddressLen())) {
            SYLAR_LOG_ERROR(g_logger)
        ...
        if (::connect_with_timeout(m_sock, addr->getAddress(),
                                   addr->getAddressLen(), timeout_ms)) {
            SYLAR_LOG_ERROR(g_logger)
                << "sock=" << m_sock << " connect(" << addr->toString()
...
int Socket::sendTo(const void* buffer, size_t length, const Address::ptr to,
                   int flags) {
    if (isConnected()) {
        return ::sendto(m_sock, buffer, length, flags, to->getAddress(),
                        to->getAddressLen());
    }
    return -1;
}
...
int Socket::recvFrom(void* buffer, size_t length, Address::ptr from,
                     int flags) {
    if (isConnected()) {
        socklen_t len = from->getAddressLen();
        return ::recvfrom(m_sock, buffer, length, flags, from->getAddress(),
                          &len);
    }
    return -1;
}
int Socket::recvFrom(iovec* buffers, size_t length, Address::ptr from,
    ...
    if (isConnected()) {
        ...
        msg.msg_namelen = from->getAddressLen();
        return ::recvmsg(m_sock, &msg, flags);
    }
    return -1;
}
...
Address::ptr Socket::getRemoteAddress() {
    ...
    socklen_t addrlen = result->getAddressLen();
    if (getpeername(m_sock, result->getAddress(), &addrlen)) {
        SYLAR_LOG_ERROR(g_logger)
            << "getpeeername error sock=" << m_sock << " errno=" << errno
...
Address::ptr Socket::getLocalAddress() {
    ...
    socklen_t addrlen = result->getAddressLen();
    if (getsockname(m_sock, result->getAddress(), &addrlen)) {
        SYLAR_LOG_ERROR(g_logger)
...
int SSLSocket::recvFrom(iovec* buffers, size_t length, Address::ptr from,
                        int flags) {
    SYLAR_ASSERT(false);
    return -1;
}
